...
| Режим         | Как работает SSL для Timeweb PRO         |
|---------------|------------------------------------------|
| Let's Encrypt | Автоматическое получение и продление     |
| Timeweb PRO   | Ручная вставка сертификата и ключа       |

### 🎉 Timeweb PRO (ручной режим)

**Внимание! Для Timeweb PRO теперь поддерживается только ручной режим.**

- Получите сертификат и приватный ключ в панели Timeweb
- Вставьте их вручную по инструкции в инсталляторе
- Автоматизация через API больше не поддерживается (Timeweb не предоставляет эндпоинтов)
...
- **v1.7.x**  
  - Только ручной режим для Timeweb PRO (вставка сертификата и ключа вручную)
  - Полностью удалена поддержка Timeweb API, токенов, ID сертификата
  - Надёжная установка зависимостей Node.js с проверкой ошибок
  - Улучшена диагностика и документация

- **v1.6.x**  
  - Поддержка Timeweb PRO через API (устарело, больше не поддерживается)
  - Автоматизация получения сертификатов через токен и ID
  - Улучшения диагностики, валидации и безопасности

- **v1.5.x**  
  - Базовая функциональность: Node.js 20 LTS, PM2, Nginx, Let's Encrypt, Timeweb PRO 
